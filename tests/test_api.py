...
def test_verify_all_responses(monkeypatch, faces, expected_status, expected_json):
    ...
    resp = client.post("/verify", json={"image": "images/simon.jpg"})
...
def test_compare_returns_true(monkeypatch):
    ...
    resp = client.post(
        "/compare", json={"image1": "images/simon.jpg", "image2": "images/olivier.jpg"}
    )
    assert resp.status_code == 200
...
def test_compare_returns_false(monkeypatch):
    ...
    resp = client.post(
        "/compare", json={"image1": "images/simon.jpg", "image2": "images/olivier2.jpg"}
    )
    assert resp.status_code == 200
